...
def _gh_get(url: str, token: Optional[str], **kwargs) -> requests.Response:
    """GET with automatic 403 rate-limit reporting."""
    resp = requests.get(url, headers=_gh_headers(token), **kwargs)
    if resp.status_code == 403:
        reset = resp.headers.get("X-RateLimit-Reset", "?")
        print(f"  GitHub rate limit hit (403) — resets at epoch {reset}. Add a valid GITHUB_TOKEN to .env.")
    return resp
    ...
    def handle(name: str, inputs: dict) -> str:
        ...
        # ── File tools ─────────────────────────────────────────
            ...
            if os.path.isabs(path):
                ...
                # Resolve relative to project_dir (the agent's working directory),
                # not the script CWD. This lets the agent write to sub-paths like
                # ".stacker/Dockerfile" correctly.
...
def _gh_check_pair(owner: str, repo: str, compose_candidates: list[str], token: Optional[str]) -> Optional[str]:
    """
    Check if owner/repo has a Dockerfile and any of the given compose paths.
    Returns the matching compose path, or None if either file is missing.
    Respects GH_DELAY between requests.
    """
    time.sleep(GH_DELAY)
    if not gh_file_exists(owner, repo, "Dockerfile", token):
        return None
    for cp in compose_candidates:
        time.sleep(GH_DELAY)
...
def find_deployable_repo(
    ...
    """
    Find a repo that has both Dockerfile and docker-compose.yml.

    Order:
      1. Known-good repos (fast, verified, uses minimal requests)
      2. Top GitHub search results (broader, but burns more rate limit)

    Pass `skip` (a set of "owner/repo" strings) to exclude repos that have
    already been deployed successfully in a previous run.
    ...
    for entry in KNOWN_REPOS.get(lang_key, []):
        ...
        if full_name in skip:
            print(f"  ↷ {full_name} (already deployed — skipping)")
            continue
        preferred = entry.get("compose_path", "docker-compose.yml")
        candidates = [preferred] + [c for c in compose_names if c != preferred]
        cp = _gh_check_pair(owner, repo, candidates, token)
        if cp:
            print(f"  ✓ {owner}/{repo} (known-good)")
            return {
    ...
    for item in candidates_list:
        if item["full_name"] in skip:
            print(f"  ↷ {item['full_name']} (already deployed — skipping)")
            continue
        owner = item["owner"]["login"]
        repo = item["name"]
        cp = _gh_check_pair(owner, repo, compose_names, token)
        mark = "✓" if cp else "✗"
        print(f"  {mark} {item['full_name']}")
        if cp:
            return {
                "owner": owner, "repo": repo,
...
def main() -> int:
    ...
    # Load set of already-deployed repos so we don't repeat successful runs.
    ...
    if args.repo:
        ...
        repo_info: dict = {
            ...
            "dockerfile_path": "Dockerfile", "compose_path": "docker-compose.yml",
        }
        print(f"  Using --repo override: {args.repo}")
    else:
        print(f"  Searching GitHub for {github_lang} web apps with Dockerfile + docker-compose…")
        repo_info = find_deployable_repo(args.lang, github_token, skip=deployed)
        if not repo_info:
    ...
    print(f"  Fetching {repo_info.get('dockerfile_path', 'Dockerfile')} and {repo_info.get('compose_path', 'docker-compose.yml')}…")

    dockerfile_content = gh_read_file(owner, repo_name, repo_info.get("dockerfile_path", "Dockerfile"), github_token)
    compose_content = gh_read_file(owner, repo_name, repo_info.get("compose_path", "docker-compose.yml"), github_token)

    if not dockerfile_content or not compose_content:
        print(f"  Dockerfile: {'✓' if dockerfile_content else '✗'}  compose: {'✓' if compose_content else '✗'}")
        print("Failed to read required files. Specify a different repo with --repo.")
        return 1

    print(f"  Dockerfile: {len(dockerfile_content)} bytes  compose: {len(compose_content)} bytes")
    ...
    deploy_result = _run(
        ...
        user_msg=(
            ...
            f"=== {repo_info.get('compose_path', 'docker-compose.yml')} ===\n"
    ...
    if run_cloud and qa_passed:
        if not hetzner_token:
            ...
            if not add_cloud_config_to_stacker_yml(project_dir, cloud_region, cloud_size, ssh_key_path):
                ...
                print("  Running: stacker deploy --target cloud --lock  (may take a few minutes)")
                rc, deploy_out = run_shell(
                    f"{stacker_cmd} deploy --target cloud --lock",
